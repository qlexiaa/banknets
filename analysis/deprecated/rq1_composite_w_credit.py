"""
rq1_composite_w_credit.py -- Composite W profile likelihood for credit growth
=============================================================================
Credit-DV parallel of rq1_composite_w.py.
...
def run(output_dir=None):
    ...
    samples = [("Full", panel), ("Non-border", panel_nb)]

    total = len(ALPHA_GRID) * len(pairs) * len(samples)
    counter = [0]
    all_results = {}
    all_rows = []
    ...
    if output_dir is not None:
        ...
        for col, pair_label in enumerate(["Pair A", "Pair B", "Pair C"]):
            for row_idx, (metric, ylabel) in enumerate([
                ...
                for sample_label, color, ls in [
                    ...
                    rows = all_results.get(pair_label, {}).get(sample_label, [])
                    ax.plot([r["alpha"] for r in rows], [r[metric] for r in rows],
                            color=color, ls=ls, lw=2, label=sample_label)
                ax.set_title(pair_titles[pair_label] if row_idx == 0 else "")
