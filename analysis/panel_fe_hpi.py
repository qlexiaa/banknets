...
def run(output_dir=None):
    ...
    W_MATRICES = [("W_geo", W_geo_all), ("W_bank", W_bank_all)]
    SAMPLE_MAP = [("Full", panel), ("Non-border", panel_nb)]
    ...
    for sample_label, _ in SAMPLE_MAP:
        ...
        r_bank = results[(sample_label, "W_bank")]
        delta, se_delta, z = gap_stats(r_geo, r_bank)
    ...
    print("=" * W)
    print(f"{'Sample':<14} {'W matrix':<8} {'N':>6} "
          f"{'beta_D':>9} {'SE':>7} {'beta_Deta':>10} {'SE':>7} "
          f"{'lambda':>9} {'SE':>7}")
    print("-" * W)
    for row in rows:
        print(f"{row['sample']:<14} {row['w_matrix']:<8} {row['n_counties']:>6} "
              f"{row['beta_D']:>9.4f} {row['beta_D_se']:>7.4f} "
              f"{row['beta_Deta']:>10.4f} {row['beta_Deta_se']:>7.4f} "
              f"{row['lambda']:>9.4f} {row['lambda_se']:>7.4f}")

    print()
    print("Lambda gap (W_bank - W_geo), one-sided H1: lambda_bank > lambda_geo")
    print(f"{'Sample':<14} {'delta':>9} {'SE':>8} {'z':>8}")
    print("-" * 38)
    for sample_label, _ in SAMPLE_MAP:
        r_bank = next(r for r in rows
                      if r["sample"] == sample_label and r["w_matrix"] == "W_bank")
        print(f"{sample_label:<14} {r_bank['delta_lambda']:>9.4f} "
              f"{r_bank['delta_lambda_se']:>8.4f} {r_bank['z_stat']:>8.3f}")
    print("=" * W)
