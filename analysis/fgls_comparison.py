...
Estimators (run for full and non-border samples):
...
def run_sample(s, sample_label, lam_geo, lam_bank, lam_star, verbose=True):
    ...
    sample_label : str  ('Full' or 'Non-border')
...
def load_lambdas(output_dir):
    ...
    return {
        ...
        "nb": {
            "geo" : get_sem_lam("FE W_geo (non-border)"),
            "bank": get_sem_lam("FE W_bank (non-border)"),
            "star": get_opt_lam("Pair A", "Non-border"),
        },
    }
