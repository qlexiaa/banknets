...
Regressions (two-way county + year FE, within estimator):
  (1) Base:    Dl_nloans_b ~ D
  (2) SLX:     Dl_nloans_b ~ D + E
  (3) Placebo: Dl_nloans_pl ~ D + E
...
from panel_data import load_panel_with_placebo, get_samples
...
def _build_sample(panel_sub, county_order, W_is_all, YEARS, year_pos,
    ...
    Excludes counties with any NaN in DV or in placebo DV.
    Returns dict with all arrays needed for estimation and SE computation.
    """
    T = len(YEARS)

    # County filter: present in sample, no NaN in either DV
    nan_dv  = panel_sub.groupby("fips5")[DV].apply(lambda s: s.isna().any())
    nan_pl  = panel_sub.groupby("fips5")[DV_PLACEBO].apply(lambda s: s.isna().any())
    sub_co  = set(panel_sub["fips5"].unique())
    usable  = [c for c in county_order
               if c in sub_co and not nan_dv.get(c, True) and not nan_pl.get(c, True)]
    N       = len(usable)
...
def _perm_test(y_TN, D_TN, E_TN, beta_E_obs):
    """
...
def _estimate_sample(samp, sample_label):
    ...
    X_3d  = np.stack([D_TN, E_TN], axis=2)   # (T, N, 2) -- original scale
    ...
    rows.append(_make_row(
        sample_label, "SLX-OLS",     DV, beta_s, se_s_ols, None, None, NT, N))
    rows.append(_make_row(
        sample_label, "SLX-Conley",  DV, beta_s, se_s_con, None, None, NT, N))
    rows.append(_make_row(
        sample_label, "SLX-Cluster", DV, beta_s, se_s_clu, None, None, NT, N))

    # -- (3) Placebo DV: yp ~ D + E -------------------------------------------
    yp_w    = _within(yp_TN).ravel()
    X_pl    = np.column_stack([D_w, E_w])
    beta_pl, u_pl, XtX_pl = _ols(yp_w, X_pl)
    se_pl = _se_ols(XtX_pl, u_pl, NT, k_slx)
    rows.append(_make_row(
        sample_label, "Placebo-OLS", DV_PLACEBO, beta_pl, se_pl, None, None, NT, N))

    # -- Permutation test on SLX beta_E ---------------------------------------
    beta_E_obs = beta_s[1]
    p_perm     = _perm_test(y_TN, D_TN, E_TN, beta_E_obs)
...
def _make_row(sample, spec, dv, beta, se, p_perm, extra, NT, N):
    ...
    row = dict(
        ...
        beta_E   = float(beta[1]) if k > 1 else np.nan,
        se_E     = float(se[1])   if k > 1 else np.nan,
        t_E      = float(beta[1] / se[1]) if (k > 1 and se[1] > 0) else np.nan,
        p_E      = float(2 * st.norm.sf(abs(beta[1] / se[1]))) if (k > 1 and se[1] > 0) else np.nan,
        p_perm   = np.nan,
    )
    return row
